...
color specularColor(const color& mat, const color& lightColor,
	...
	const dvec3& r, const dvec3& v) {
	/* CSE 386 - todo  */
    double dp = glm::max(glm::dot(r, v), 0.0);
	return glm::clamp(mat * lightColor * glm::pow(dp, shininess), 0.0, 1.0);
}
...
color totalColor(const Material& mat, const color& lightColor,
    ...
    color B = diffuseColor(mat.diffuse, lightColor, l, n);
    color S = specularColor(mat.specular, lightColor, mat.shininess, 2 * glm::dot(l, n) * n - l, v);
    // add atteniation
    if (attenuationOn) {
        return glm::clamp(A + ATparams.factor(glm::distance(lightPos, intersectionPt)) * (B + S), 0.0, 1.0);
    } else {
        return glm::clamp(A + B + S, 0.0, 1.0);
    }
}
...
bool PositionalLight::pointIsInAShadow(const dvec3& intercept,
	...
	/* CSE 386 - todo  */
    dvec3 adjustedIntercept = IShape::movePointOffSurface(intercept, normal);
    Ray shadowFeeler = getShadowFeeler(adjustedIntercept, normal, eyeFrame);
    OpaqueHitRecord hit;
    VisibleIShape::findIntersection(shadowFeeler, objects, hit);
    if (hit.t == FLT_MAX) {
        return false;
    } else if (glm::distance(hit.interceptPt, adjustedIntercept) < glm::distance(adjustedIntercept, pos)) {
        return true;
    }
	return false;
}
...
//	dvec3 origin(0, 0, 0);
//	dvec3 dir(1, 1, 1);
	Ray shadowFeeler(interceptWorldCoords, glm::normalize(pos - interceptWorldCoords));
	return shadowFeeler;
}
...
bool SpotLight::isInSpotlightCone(const dvec3& spotPos,
	...
	/* CSE 386 - todo  */
    if (glm::dot(glm::normalize(intercept - spotPos), spotDir) >= glm::cos(spotFOV/2)) {
        return true;
    }
	return false;
}
